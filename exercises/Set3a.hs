...
firstHalf str
 ...
 | otherwise = take (div (length str) 2) str
...
capitalizeFirst str = case str of 
    [] -> ""
    (x:xs) -> toUpper x : xs
...
joinToLength n list = [x1 ++ x2 | x1 <- list, x2 <- list, length (x1 ++ x2) == n]
...
sumRights a = case a of
    [] -> 0
    (Right a : xs) -> a + sumRights xs
    (_  : xs) -> sumRights xs
...
multiCompose fs a = foldr (\f acc -> f acc) a fs
...
interpreter commands = helper commands (0,0)
 where 
    ...
    helper list (n, m) = case list of
        [] -> []
        ("up":xs) -> helper xs (n, m+1)
        ("down":xs) -> helper xs (n, m-1)
        ("right":xs) -> helper xs (n+1, m)
        ("left":xs) -> helper xs (n-1, m)
        ("printX":xs) -> show n : helper xs (n, m)
        ("printY":xs) -> show m : helper xs (n, m)