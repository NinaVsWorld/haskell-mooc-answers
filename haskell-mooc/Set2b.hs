...
leftpad s n = concat (replicate (n - (length s)) " ") ++ s
...
countdown n = "Ready! " ++ concatMap ((++"... ") . show) (reverse [1..n]) ++ "Liftoff!"
...
smallestDivisor x = go x 2

go :: Integer -> Integer -> Integer
go x n = if x `mod` n == 0 then n else go x (n + 1)
------------------------------------------------------------------------------
